...
class twitter{
	 ...
	 */
	function update( $status, $url = false, $replying_to = false )
	{
	    if( !in_array( $this->type, array( 'xml','json' ) ) )
	        return false;
	    
	    if($url){
	    	require_once('../classes/stuffBitly.php');
			$bitly = new stuffBitly();
			$bitlyResult = $bitly->shortenUrl($url);
			//need to count how many chars in $status then append it to the end with ... if we have more than 140 chars now
			$status = $status .= ' '.$bitlyResult;
	    }
